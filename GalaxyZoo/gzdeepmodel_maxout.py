...
def get_maxout(dim_input):
    config = {
        ...
        'layers': [
        MaxoutConvC01B(layer_name='h0', num_channels=64, num_pieces=2, irange=.005, tied_b=1, max_kernel_norm=.9,
                       kernel_shape=[7, 7], pool_shape=[4, 4], pool_stride=[2, 2], W_lr_scale=.05, b_lr_scale=.05),
        MaxoutConvC01B(layer_name='h1', num_channels=128, num_pieces=2, irange=.005, tied_b=1, max_kernel_norm=0.9,
                       kernel_shape=[7, 7], pool_shape=[4, 4], pool_stride=[2, 2], W_lr_scale=.05, b_lr_scale=.05),
        MaxoutConvC01B(layer_name='h2', num_channels=192, num_pieces=4, irange=.005, tied_b=1, max_kernel_norm=0.9,
                       kernel_shape=[5, 5], pool_shape=[2, 2], pool_stride=[2, 2], W_lr_scale=.05, b_lr_scale=.05),
        Maxout(layer_name='h3', irange=.005, num_units=500, num_pieces=5, max_col_norm=1.9),
...
def get_trainer(model, trainset, epochs=50):
    ...
    path = DATA_DIR + 'model' + str(SUBMODEL) + 'saved_maxout.pkl'
...
if __name__ == '__main__':
    ...
    if os.path.exists(DATA_DIR + 'model' + str(SUBMODEL) + 'saved_maxout.pkl'):
        model = serial.load(DATA_DIR + 'model' + str(SUBMODEL) + 'saved_maxout.pkl')
        iters = 1
    ...
    np.save(DATA_DIR+'feats_maxout_train' + str(SUBMODEL) + '.npy', outtrainset)

    outtestset = get_output(model, testset)
    np.save(DATA_DIR+'feats_maxout_test' + str(SUBMODEL) + '.npy', outtestset)
